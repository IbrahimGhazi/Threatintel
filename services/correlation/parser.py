...
# These services generate massive volumes of traffic and would waste TI lookup
# API calls while never being actual threat indicators.
ALLOWLISTED_CIDRS = [
    ...
    ipaddress.ip_network("103.152.101.0/24"),    # NTP pool servers seen in environment
    # Google (Search, YouTube, APIs, Cloud)
    ipaddress.ip_network("142.250.0.0/15"),      # Google services
    ipaddress.ip_network("172.217.0.0/16"),      # Google services
    ipaddress.ip_network("216.58.0.0/16"),       # Google services
    ipaddress.ip_network("74.125.0.0/16"),       # Google services
    ipaddress.ip_network("64.233.160.0/19"),     # Google services
    ipaddress.ip_network("34.0.0.0/8"),          # Google Cloud
    # Microsoft (Azure, O365, Bing, Teams)
    ipaddress.ip_network("13.107.0.0/16"),       # Microsoft
    ipaddress.ip_network("40.99.0.0/16"),        # Microsoft O365
    ipaddress.ip_network("40.126.0.0/16"),       # Microsoft
    ipaddress.ip_network("52.96.0.0/14"),        # Microsoft O365
    ipaddress.ip_network("20.190.0.0/16"),       # Microsoft Azure AD
    ipaddress.ip_network("204.79.197.0/24"),     # Bing
    ipaddress.ip_network("150.171.0.0/16"),      # Microsoft
    # Cloudflare (CDN, WAF)
    ...
    # Apple
    ipaddress.ip_network("17.248.0.0/14"),       # Apple services
    # Akamai CDN
    ipaddress.ip_network("23.206.0.0/16"),       # Akamai
    ipaddress.ip_network("95.101.0.0/16"),       # Akamai
    # Fastly CDN
    ipaddress.ip_network("151.101.0.0/16"),      # Fastly / Reddit
    # AWS common ranges
    ipaddress.ip_network("65.0.0.0/16"),         # AWS
    ipaddress.ip_network("43.205.0.0/16"),       # AWS India
    ipaddress.ip_network("143.204.0.0/16"),      # AWS CloudFront
    # Canonical / Ubuntu updates
    ipaddress.ip_network("185.125.190.0/24"),    # Canonical
]
